...
public class EmailService {
    ...
    public void sendBookingUpdateEmail(Booking booking, String customerEmail) {
        try {
            System.out.println("📧 Sending booking update email to: " + customerEmail);

            MimeMessage mimeMessage = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true, "UTF-8");

            helper.setTo(customerEmail);
            helper.setSubject("✏️ Booking Updated - " + booking.getBookingReference());
            helper.setFrom("goldpalmhotelsliit@gmail.com");
            helper.setText(createBookingUpdateHtml(booking), true);

            mailSender.send(mimeMessage);
            System.out.println("✅ Booking update email sent to: " + customerEmail);

        } catch (Exception e) {
            System.err.println("❌ Error sending booking update email: " + e.getMessage());
            e.printStackTrace();
        }
    }

    private String createBookingUpdateHtml(Booking booking) {
        return String.format(
            "<!DOCTYPE html>" +
            "<html>" +
            "<head>" +
            ...
            "        .header { background: linear-gradient(135deg, #3498db, #2c3e50); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }" +
            "        .content { background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px; }" +
            "        .update-notice { background: #d1ecf1; color: #0c5460; padding: 20px; margin: 20px 0; border-radius: 8px; border: 1px solid #bee5eb; text-align: center; }" +
            "        .booking-details { background: white; padding: 20px; margin: 20px 0; border-radius: 8px; border-left: 4px solid #3498db; }" +
            "        .detail-row { margin: 10px 0; padding: 8px 0; border-bottom: 1px solid #eee; }" +
            "        .label { font-weight: bold; color: #2c3e50; }" +
            "        .value { color: #34495e; }" +
            ...
            "                <p>Your booking details have been updated as requested. Please review the updated information below.</p>" +
            "            </div>" +
            "            " +
            "            <div class='booking-details'>" +
            ...
            "                    %s" +
            "                </table>" +
            "                <div class='total' style='font-size: 1.2em; font-weight: bold; color: #27ae60; background: #e8f5e8; padding: 15px; border-radius: 5px; text-align: center; margin-top: 15px;'>💰 Total Amount: LKR %.2f</div>" +
            "            </div>" +
            "            " +
            "            <div class='footer'>" +
            "                <p><strong>What's Next?</strong></p>" +
            "                <p>Your booking is confirmed with the updated details. We look forward to welcoming you!</p>" +
            "                <p style='margin-top: 30px;'><em>Thank you for choosing Gold Palm Hotel!</em></p>" +
            "                <p style='font-size: 0.9em; margin-top: 20px;'>" +
            "                    Questions? Contact us at <a href='mailto:support@goldpalmhotel.com'>support@goldpalmhotel.com</a>" +
            "                </p>" +
            ...
            booking.getSpecialRequests() != null && !booking.getSpecialRequests().trim().isEmpty() ?
                String.format("<tr class='detail-row'><td class='label' style='font-weight: bold; color: #2c3e50; width: 50%;'>📝 Special Requests:</td><td class='value' style='color: #34495e; text-align: right;'>%s</td></tr>",
                            booking.getSpecialRequests()) : "",
            booking.getCustomerNotes() != null && !booking.getCustomerNotes().trim().isEmpty() ?
                String.format("<tr class='detail-row'><td class='label' style='font-weight: bold; color: #2c3e50; width: 50%;'>📝 Your Notes:</td><td class='value' style='color: #34495e; text-align: right;'>%s</td></tr>",
