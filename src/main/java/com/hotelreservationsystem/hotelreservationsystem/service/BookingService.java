...
public class BookingService {
    ...
    public BookingResponseDTO updateBookingDetails(Long id, Map<String, Object> updateData) {
        ...
        try {
            String customerEmail = updatedBooking.getCustomer().getUser().getEmail();
            System.out.println("📧 Sending booking update email to: " + customerEmail);
            emailService.sendBookingUpdateEmail(updatedBooking, customerEmail);
        } catch (Exception e) {
            System.err.println("⚠️ Failed to send update email: " + e.getMessage());
    ...
    /**
     * Delete a booking - used when deleting user account
     */
    public void deleteBooking(Long bookingId) {
        Booking booking = bookingRepository.findById(bookingId)
                .orElseThrow(() -> new RuntimeException("Booking not found with ID: " + bookingId));

        System.out.println("Deleting booking: " + booking.getBookingReference());
